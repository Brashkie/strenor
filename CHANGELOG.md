...
- **Sets** (unique members): `sadd`, `srem`, `sismember`, `smembers`, `scard`.
  Members reuse the tagged value format and codecs; uniqueness is by exact
  serialized bytes. Journalled to the AOF and included in snapshots; an emptied
  set key is removed (Redis-like).

### Changed

- Snapshot format bumped to v5 (adds the set type); v1–v4 snapshots still load.
